...
public class VsCodeBridgeService : IAsyncDisposable
{
    ...
    public async Task<T?> SendAsync<T>(string command, object? payload = null, TimeSpan? timeout = null)
    {
        ...
        await _js.InvokeVoidAsync("bridge.send", new { requestId = id, command, payload });
